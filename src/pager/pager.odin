...
find_empty_slot :: proc(p: ^Pager) -> ^Page_Slot {
	...
	for i in 0 ..< PAGE_CACHE_SIZE {
		slot := &p.slots[i]
		if slot.page.page_num == 0 { slot.page.data = slot._data_buf[:]; p.slot_count += 1; return slot }
	}
...
evict_one_slot :: proc(p: ^Pager) -> Error {
	for i in 0 ..< PAGE_CACHE_SIZE {
		...
		if slot.page.page_num != 0 && slot.page.pin_count == 0 {
			if slot.page.dirty { if err := flush_page_unsafe(p, &slot.page); err != .None { return err } }
			delete_key(&p.cache_index, slot.page.page_num)
			slot.page.page_num = 0; slot.page.data = nil; p.slot_count -= 1; return .None
		}
	}
	return .Cache_Full
}
...
open :: proc(path: string, max_pages: u32 = 256, allocator := context.allocator) -> (^Pager, Error) {
	...
	if open_err != nil { free(p); return nil, .File_Open_Failed }
	p.file = file
	file_size, size_err := os.file_size(file)
	if size_err != nil { os.close(file); free(p); return nil, .IO_Error }
...
close :: proc(p: ^Pager) -> Error {
	...
	delete(p.cache_index); free(p, p.allocator)
	return flush_err
}
...
get_page :: proc(p: ^Pager, page_num: u32) -> (^Page, Error) {
	...
	if slot := find_slot(p, page_num); slot != nil { slot.page.pin_count += 1; return &slot.page, .None }
...
unpin_page :: proc(p: ^Pager, page_num: u32) {
	sync.rw_mutex_lock(&p.mutex); defer sync.rw_mutex_unlock(&p.mutex)
	if slot := find_slot(p, page_num); slot != nil && slot.page.pin_count > 0 { slot.page.pin_count -= 1 }
}
