...
Config :: struct {
	...
	zero_copy: bool, // If true, strings/blobs point into the source buffer.
}

DEFAULT_CONFIG := Config {
	allocator = {}, // Defaults to context.allocator
	zero_copy = false,
}
...
deserialize :: proc(
	...
	for st in serial_types {
		...
		if st == u64(types.Serial_Type.NULL) {
			...
			if pos + text_len > len(src) { return {}, 0, false }
...
debug_print :: proc(c: Cell) {
	fmt.printf("Cell(rowid=%d, values=[", c.rowid)
...
// Validates that the cell's values match the expected table schema.
