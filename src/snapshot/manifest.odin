...
Table_Change :: enum { CREATED, DROPPED, MODIFIED }
...
create_manifest :: proc(p: ^pager.Pager, tables: []types.Table) -> u32 {
	...
	if err != .None { fmt.eprintln("Snapshot: failed to allocate manifest page"); return 0 }
	data := page.data
	offset := 0
	copy(data[offset:], MANIFEST_MAGIC); offset += len(MANIFEST_MAGIC)
	count := u32(len(tables))
	mem.copy_non_overlapping(raw_data(data[offset:]), &count, size_of(count)); offset += size_of(count)
	for tbl in tables {
		entry := Manifest_Entry{name_hash = u64(types.hash_string(tbl.name)), root_page = tbl.root_page, name_len = u16(len(tbl.name))}
		mem.copy_non_overlapping(raw_data(data[offset:]), &entry, size_of(entry)); offset += size_of(entry)
		copy(data[offset:], transmute([]u8)tbl.name); offset += len(tbl.name)
	}
	pager.mark_dirty(p, page.page_num); pager.unpin_page(p, page.page_num)
	return page.page_num
}

find_in_manifest :: proc(p: ^pager.Pager, manifest_page: u32, table_name: string) -> (root_page: u32, ok: bool) {
	...
	for _ in 0 ..< count {
		...
		if entry.name_hash == target_hash && string(data[offset:offset + int(entry.name_len)]) == table_name { return entry.root_page, true }
		offset += int(entry.name_len)
	}
	return 0, false
}

load_manifest :: proc(p: ^pager.Pager, manifest_page: u32, allocator := context.allocator) -> []Manifest_Entry {
...
diff_manifests :: proc(p: ^pager.Pager, manifest_a: u32, manifest_b: u32, allocator := context.allocator) -> ([]Snapshot_Diff_Entry, bool) {
	...
	for i in 0 ..< len(new_names) {
		...
		for j in 0 ..< len(old_names) {
			if new_names[i] == old_names[j] {
				found = true
				if new_roots[i] != old_roots[j] { append(&entries, Snapshot_Diff_Entry{table_name = strings.clone(new_names[i], allocator), change = .MODIFIED, old_root = old_roots[j], new_root = new_roots[i]}) }
				break
			}
		}
		if !found { append(&entries, Snapshot_Diff_Entry{table_name = strings.clone(new_names[i], allocator), change = .CREATED, new_root = new_roots[i]}) }
	}
	for j in 0 ..< len(old_names) {
		found := false
		for i in 0 ..< len(new_names) { if old_names[j] == new_names[i] { found = true; break } }
		if !found { append(&entries, Snapshot_Diff_Entry{table_name = strings.clone(old_names[j], allocator), change = .DROPPED, old_root = old_roots[j]}) }
	}
	return entries[:], true
}

diff_snapshots :: proc(p: ^pager.Pager, older_id: u64, newer_id: u64, latest_page: u32, allocator := context.allocator) -> ([]Snapshot_Diff_Entry, bool) {
