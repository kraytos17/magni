...
begin_impl :: proc(db: ^Database) -> bool {
	if db.txn_state == .ACTIVE { fmt.eprintln("Warning: Transaction already in progress"); return false }
	db.txn_state = .ACTIVE
...
commit_impl :: proc(db: ^Database) -> bool {
	if db.txn_state != .ACTIVE { fmt.eprintln("Warning: No active transaction to commit"); return false }
	...
	snap_page, snap_ok := snapshot.create(db.pager, snap_id, db.latest_snapshot, db.schema_root_page, manifest_page, .COMMIT)
	if !snap_ok { fmt.eprintln("Error: Failed to create snapshot"); return false }
...
rollback_impl :: proc(db: ^Database) -> bool {
	if db.txn_state != .ACTIVE { fmt.eprintln("Warning: No active transaction to roll back"); return false }
