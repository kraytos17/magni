...
execute :: proc(db: ^Database, sql: string) -> bool {
	...
	if !ok { fmt.eprintln("Error: Failed to parse SQL statement"); return false }
	if txn_stmt, is_txn := stmt.type.(parser.Txn_Stmt); is_txn {
		switch txn_stmt.op {
		case .BEGIN: return begin_impl(db)
		case .COMMIT: return commit_impl(db)
		case .ROLLBACK: return rollback_impl(db)
		}
	}
	st := schema_tree(db)
	...
	if sel, is_sel := stmt.type.(parser.Select_Stmt); is_sel {
		if snap_id, has_snap := sel.as_of_snapshot.?; has_snap {
			...
			if !has_page { fmt.eprintln("Error: Snapshot", snap_id, "not found"); return false }
			snap_h, snap_ok := snapshot.load(db.pager, snap_page)
			if !snap_ok { fmt.eprintln("Error: Failed to load snapshot", snap_id); return false }
			st.root = snap_h.schema_root; as_of_override = true
			...
			if !snap_ok { fmt.eprintln("Error: No snapshot found at or before timestamp", ts_val); return false }
			st.root = snap_h.schema_root; as_of_override = true
		}
	}
	...
	if exec_ok && db.txn_state == .NONE && !as_of_override {
		...
		case parser.Insert_Stmt: snap_op = .INSERT
		case parser.Update_Stmt: snap_op = .UPDATE
		case parser.Delete_Stmt: snap_op = .DELETE
		case parser.Create_Stmt: snap_op = .CREATE
		case parser.Drop_Stmt:  snap_op = .DROP
		}
		mt := executor.mutated_table_info
		if mt.name != "" {
			if mt.root != 0 { db.table_roots[mt.name] = mt.root }
			else { delete_key(&db.table_roots, mt.name) }
			executor.mutated_table_info = {}
		} else if snap_op == .CREATE || snap_op == .DROP { db.table_roots_dirty = true }
		ensure_table_roots(db)
		tables := make([dynamic]types.Table, context.temp_allocator)
		for name, root in db.table_roots { append(&tables, types.Table{name = name, root_page = root}) }
		...
		snap_page, snap_ok := snapshot.create(db.pager, snap_id, db.latest_snapshot, db.schema_root_page, manifest_page, snap_op)
		if snap_ok {
			...
			if db.gc_pending_count >= GC_INTERVAL { snapshot.gc(db.pager, db.latest_snapshot, 100); db.gc_pending_count = 0 }
		}
...
query :: proc(db: ^Database, sql: string) -> Query_Result {
	...
	if !parse_ok { fmt.eprintln("Error: Failed to parse SQL statement"); return r }
	st := schema_tree(db)
	if sel, is_sel := stmt.type.(parser.Select_Stmt); is_sel {
		if snap_id, has_snap := sel.as_of_snapshot.?; has_snap {
			snap_page, has_page := db.snapshot_index[snap_id]
			if !has_page { fmt.eprintln("Error: Snapshot", snap_id, "not found"); return r }
			snap_h, snap_ok := snapshot.load(db.pager, snap_page)
			if !snap_ok { return r }
			st.root = snap_h.schema_root
		} else if ts_val, has_ts := sel.as_of_timestamp.?; has_ts {
			snap_h, snap_ok := snapshot.find_by_timestamp(db.pager, db.latest_snapshot, ts_val)
			if !snap_ok { fmt.eprintln("Error: No snapshot found at or before timestamp", ts_val); return r }
			st.root = snap_h.schema_root
		}
		...
		for col, i in cols { col_names[i] = col.name; col_types[i] = col.type }
