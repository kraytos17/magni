...
integrity_check :: proc(db: ^Database) -> bool {
	...
	for table in tables {
		...
		if page_err != .None { fmt.printf("✗ Table '%s' root page %d is missing\n", table.name, table.root_page); return false }
...
describe_table :: proc(db: ^Database, table_name: string) -> bool {
	...
	if !found { fmt.eprintln("Error: Table not found:", table_name); return false }
...
stats :: proc(db: ^Database) {
	...
	fmt.printf("Database size: %d bytes (%.2f KB)\n", page_count * u32(types.PAGE_SIZE), f64(page_count * u32(types.PAGE_SIZE)) / 1024.0)
	st := schema_tree(db)
	tables := schema.list_tables(&st, context.temp_allocator)
...
dump_table :: proc(db: ^Database, table_name: string) {
	...
	st := schema_tree(db)
	table, found := schema.find_table(&st, table_name, context.temp_allocator)
	if !found { fmt.printf("Error: Table '%s' not found.\n", table_name); return }
	...
	if err != .None { fmt.println("Error: Could not start cursor", err); return }
	defer btree.cursor_destroy(&cursor)
	row_count := 0
	for cursor.is_valid {
		c, get_err := btree.cursor_get_cell(&cursor, context.temp_allocator)
		if get_err != .None { fmt.printf("Error reading cell: %v\n", get_err); btree.cursor_advance(&cursor); continue }
		fmt.printf("Row %d [RowID=%d]: ", row_count + 1, c.rowid)
		for val, i in c.values {
			if i > 0 do fmt.print(", ")
			col_name := "?"; if i < len(table.columns) { col_name = table.columns[i].name }
