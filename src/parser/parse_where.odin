...
parse_value :: proc(p: ^Parser, allocator := context.allocator) -> (val: types.Value, ok: bool) {
	...
	case .NUMBER:
		advance(p)
		if strings.contains(token.lexeme, ".") { v := strconv.parse_f64(token.lexeme) or_return; return types.value_real(v), true }
		else { v := strconv.parse_i64(token.lexeme) or_return; return types.value_int(v), true }
	case .STRING:
...
parse_where_clause :: proc(p: ^Parser, allocator := context.allocator) -> (clause: Maybe(Where_Clause), ok: bool) {
	w := Where_Clause{is_and = true}
	...
	for {
		cond := Condition{column = parse_qualified_identifier(p, allocator) or_return}
		op_token := peek(p)
		...
		case .IN:
			...
			if !match(p, .LPAREN) { delete(cond.column, allocator); return nil, false }
			if peek(p).type == .SELECT {
				subq_variant, subq_ok := parse_select(p, allocator)
				if !subq_ok { delete(cond.column, allocator); return nil, false }
				subq_ptr := new(Select_Stmt, allocator)
				subq_ptr^ = subq_variant.(Select_Stmt)
				cond.in_subquery = subq_ptr
				if !match(p, .RPAREN) { delete(cond.column, allocator); return nil, false }
			} else {
				in_vals := make([dynamic]types.Value, allocator)
				for {
					val, val_ok := parse_value(p, allocator)
					if !val_ok { for v in in_vals { types.value_delete(v, allocator) }; delete(in_vals); delete(cond.column, allocator); return nil, false }
					append(&in_vals, val)
					if match(p, .RPAREN) { break }
					if !match(p, .COMMA) { for v in in_vals { types.value_delete(v, allocator) }; delete(in_vals); delete(cond.column, allocator); return nil, false }
				}
				cond.in_values = in_vals[:]
			}
		case: delete(cond.column, allocator); return nil, false
		}
		if cond.operator != .IN {
			if peek(p).type == .IDENTIFIER { cond.rhs = parse_qualified_identifier(p, allocator) or_return }
			else { val, val_ok := parse_value(p, allocator); if !val_ok { delete(cond.column, allocator); return nil, false }; cond.rhs = val }
		}
		append(&conditions, cond)
		if match(p, .AND) {
			if !first_logical_op_seen { w.is_and = true; first_logical_op_seen = true }
			else if !w.is_and { return nil, false }
		} else if match(p, .OR) {
			if !first_logical_op_seen { w.is_and = false; first_logical_op_seen = true }
			else if w.is_and { return nil, false }
		} else { break }
	}
