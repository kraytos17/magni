...
statement_free :: proc(stmt: Statement, allocator := context.allocator) {
	...
	case Create_Stmt:
		...
		for fk in s.foreign_keys { delete(fk.col, allocator); delete(fk.ref_table, allocator); delete(fk.ref_col, allocator) }
	...
	case Select_Stmt:
		...
		case string: delete(src, allocator)
		case ^Select_Stmt: statement_free(Statement{type = src^, sql = ""}, allocator); free(src, allocator)
		}
		if s.from_alias != "" { delete(s.from_alias, allocator) }
		for j in s.joins {
			#partial switch j_src in j.source {
			case string: delete(j_src, allocator)
			case ^Select_Stmt: statement_free(Statement{type = j_src^, sql = ""}, allocator); free(j_src, allocator)
			}
		...
		if order, ok := s.order_by.?; ok { for o in order { delete(o.column, allocator) }; delete(order, allocator) }
