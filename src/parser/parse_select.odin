...
is_subquery_start :: proc(p: ^Parser) -> bool {
	return peek(p).type == .LPAREN && p.current + 1 < len(p.tokens) && p.tokens[p.current + 1].type == .SELECT
}
...
parse_single_join :: proc(p: ^Parser, allocator := context.allocator, join_type: Join_Type, on_required: bool) -> (jc: Join_Clause, ok: bool) {
	js := parse_join_source(p, allocator)
	if !js.success { return {}, false }
	on_cl: Maybe(Where_Clause)
	if on_required { if !match(p, .ON) { return {}, false }; on_cl, ok = parse_where_clause(p, allocator); if !ok { return {}, false } }
	else if match(p, .ON) { on_cl, ok = parse_where_clause(p, allocator); if !ok { return {}, false } }
...
parse_select_columns :: proc(p: ^Parser, columns: ^[dynamic]string, aggregates: ^[dynamic]Aggregate_Expr, allocator := context.allocator) -> bool {
	if match(p, .ASTERISK) { } else {
		for {
			tok := peek(p)
			if tok.type == .IDENTIFIER && p.current + 1 < len(p.tokens) && p.tokens[p.current + 1].type == .LPAREN {
				func_name_upper := strings.to_upper(tok.lexeme, context.temp_allocator)
				agg_func: Aggregate_Func
				agg_ok := false
				switch func_name_upper {
				case "COUNT": agg_func = .COUNT; agg_ok = true
				case "SUM": agg_func = .SUM; agg_ok = true
				case "AVG": agg_func = .AVG; agg_ok = true
				case "MIN": agg_func = .MIN; agg_ok = true
				case "MAX": agg_func = .MAX; agg_ok = true
				}
				if !agg_ok {
					col, cok := parse_identifier(p, allocator); if !cok { return false }
					append(columns, col)
				} else {
					advance(p); advance(p)
					is_star := match(p, .ASTERISK)
					arg_col: string
					if !is_star { var, acok := parse_qualified_identifier(p, allocator); if !acok { return false }; arg_col = var }
				...
				col, cok := parse_qualified_identifier(p, allocator); if !cok { return false }
				append(columns, col)
			}
...
parse_join_clauses :: proc(p: ^Parser, allocator := context.allocator) -> [dynamic]Join_Clause {
	...
	for {
		if match(p, .COMMA) { jc, jc_ok := parse_single_join(p, allocator, .CROSS, false); if !jc_ok { break }; append(&joins, jc) }
		else if match(p, .JOIN) { jc, jc_ok := parse_single_join(p, allocator, .INNER, false); if !jc_ok { break }; append(&joins, jc) }
		else if match(p, .INNER) { if !match(p, .JOIN) { break }; jc, jc_ok := parse_single_join(p, allocator, .INNER, true); if !jc_ok { break }; append(&joins, jc) }
		else if match(p, .CROSS) { if !match(p, .JOIN) { break }; jc, jc_ok := parse_single_join(p, allocator, .CROSS, false); if !jc_ok { break }; append(&joins, jc) }
		else if match(p, .LEFT) { match(p, .OUTER); if !match(p, .JOIN) { break }; jc, jc_ok := parse_single_join(p, allocator, .LEFT, true); if !jc_ok { break }; append(&joins, jc) }
		else { break }
	}
	return joins
}
...
parse_select :: proc(p: ^Parser, allocator := context.allocator) -> (stmt: Statement_Variant, ok: bool) {
	...
	js := parse_join_source(p, allocator); if !js.success { return nil, false }
	...
	if match(p, .AS) && match(p, .OF) {
		if match(p, .SNAPSHOT) { id_token := expect(p, .NUMBER) or_return; as_of_snapshot = strconv.parse_u64(id_token.lexeme) or_return }
		else if match(p, .TIMESTAMP) { id_token := expect(p, .NUMBER) or_return; as_of_timestamp = strconv.parse_u64(id_token.lexeme) or_return }
	}
	...
	if match(p, .GROUP) { if !match(p, .BY) { return nil, false }; for { append(&group_by, parse_qualified_identifier(p, allocator) or_return); if !match(p, .COMMA) { break } } }
	...
	if match(p, .LIMIT) {
		...
		if match(p, .OFFSET) { offset_token := expect(p, .NUMBER) or_return; offset = strconv.parse_u64(offset_token.lexeme) or_return }
	}
	return Select_Stmt{from = from_val, from_alias = from_alias, joins = joins[:], columns = columns[:], aggregates = aggregates[:], is_distinct = is_distinct, where_clause = where_clause, order_by = order_by, limit = limit, offset = offset, group_by = group_by[:], having = having_cl, as_of_snapshot = as_of_snapshot, as_of_timestamp = as_of_timestamp}, true
}
