...
split_leaf_node :: proc(t: ^Tree, curr: ^Node) -> (Split_Result, Error) {
	...
	mid := total / 2
	if !node_move_leaf_cells(curr, &right_node, mid, total - mid) { return {}, .Serialization_Failed }
	if mid > 0 {
...
split_interior_root :: proc(t: ^Tree, split: Split_Result) -> Error {
	...
	if !node_move_interior_cells(&root_node, &left_node, 0, total) { unpin_node(t, root_node); return .Serialization_Failed }
