...
tree_delete_cow :: proc(t: ^Tree, key: types.Row_ID) -> (new_root: u32, err: Error) {
	...
	delete_cow_recursive :: proc(
		...
		page_id := pid
		if cow { new_id, c_err := copy_on_write(t, page_id); if c_err != .None { return {}, c_err }; page_id = new_id }
		node, n_err := load_node(t, page_id)
		if n_err != .None { return {}, n_err }
		defer unpin_node(t, node)
		if is_leaf(node) { return Update_COW_Result{new_page = node.id}, delete_from_leaf(t, &node, key) }
		child_id := node_find_child(&node, key)
		child_result, c_err := delete_cow_recursive(t, child_id, key, true)
		if c_err != .None { return {}, c_err }
		if child_result.new_page !=
		   child_id { node_update_child_ptr(&node, child_id, child_result.new_page) }
...
tree_update_cow :: proc(
	...
	update_recursive :: proc(
		...
		defer unpin_node(t, node)
		if is_leaf(node) {
			if d_err := delete_from_leaf(t, &node, rowid); d_err != .None { return {}, d_err }
			if i_err := node_insert_leaf_cell(t, &node, rowid, values); i_err != .None { return {}, i_err }
